...
def withBuildSetupUpdate(j: Job) = j.withSteps(j.steps.flatMap(updatedBuildSetupStep) *)
...
inThisBuild(
  List(
    ...
    homepage     := Some(uri("https://anymindgroup.com")),
    developers   := List(
      Developer(id = "rolang", name = "Roman Langolf", email = "rolang@pm.me", url = uri("https://github.com/rolang")),
      Developer(
        id = "dutch3883",
        name = "Panuwach Boonyasup",
        email = "dutch3883@hotmail.com",
        url = uri("https://github.com/dutch3883"),
      ),
      Developer(
        id = "qhquanghuy",
        name = "Huy Nguyen",
        email = "huy_ngq@flinters.vn",
        url = uri("https://github.com/qhquanghuy"),
      ),
    ),
    scala3             := _scala3,
    ...
    ciReleaseJobs := ciReleaseJobs.value
      ...
      .map(j =>
        j.withSteps(
          j.steps.map {
            case Step.SingleStep(name @ "Release", _, _, _, _, _, env, _) =>
              Step.SingleStep(
                name = name,
                run = Some(
