...
object express_vertex_ai extends ZIOAppDefault:
  def run = for
    ...
    txt2          <- express.generateTextFromText("hello again")
    _             <- ZIO.logInfo("Text response (from text): " + txt2)
    joke          <- express.generateStructured[Joke](msg)
    _             <- ZIO.logInfo("Joke: " + joke.setup + " \u2014 " + joke.punchline)
    joke2         <- express.generateStructuredFromText[Joke]("Tell me another joke")
    _             <- ZIO.logInfo("Joke (from text): " + joke2.setup + " \u2014 " + joke2.punchline)
  yield ()
