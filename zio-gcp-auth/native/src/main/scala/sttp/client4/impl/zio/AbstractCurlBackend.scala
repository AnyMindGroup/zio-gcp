...
//   override implicit val monad: MonadError[F] = _monad
...
//     println(s"allocated zone ${zone.hashCode()}: ${zone.isOpen}")
...
//     def close() = {
//       println(s"closing zone ${zone.hashCode()}: ${zone.isOpen}")
//       zone.close()
...
//       val ctx = new Context()
//       monad.ensure(body(ctx), monad.eval(ctx.close()))
//     }
//   }
...
// /**
//  * Curl backends that performs the curl operation with a simple
//  * `curl_easy_perform`.
//  */
